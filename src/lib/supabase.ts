...
export const saveNote = action(async (id: string | undefined, formData: FormData) => {
  ...
  const post = await fetchNoteById(id ?? '');

  let { data, error } = await supabase
  .from('post')
  .update({ title: title, description: body, update_count: post.update_count + 1 })
  .eq('id', id)
  .select()
